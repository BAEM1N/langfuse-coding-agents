...
- **NEW (v0.1.0):** Codex CLI 0.128+ native hook system support via
  `codex-native-bridge.py`. Installer auto-wires `~/.codex/hooks.json` for
  `SessionStart` / `PreToolUse` / `PostToolUse` / `UserPromptSubmit` / `Stop`
  events — no oh-my-codex hook plugin required.
...
cd langfuse-coding-agents/tools/oh-my-codex
bash install.sh
```
...
## Codex CLI native hook mode

Codex CLI 0.128+ ships its own hook system at `~/.codex/hooks.json` with
8 event types (`PreToolUse`, `PostToolUse`, `Stop`, `SessionStart`,
`UserPromptSubmit`, `PermissionRequest`, `PreCompact`, `PostCompact`).
The bundled `codex-native-bridge.py` translates Codex's stdin payload
(`hook_event_name` / `sessionId` / `toolName` / ...) into the omx native
event shape (`event` / `session_id` / `tool_name` / ...) that
`langfuse_hook.py` expects, then forwards it.

`install.sh` registers the bridge automatically alongside any existing
entries (e.g. `oh-my-codex`'s `codex-native-hook.js`) — both run side by
side. Re-running the installer is idempotent.

### Manual wiring

Add this matcher group inside each event in `~/.codex/hooks.json`:
...
```json
{
  "hooks": [
    {
      "type": "command",
      "command": "/usr/bin/python3 \"/Users/<you>/.omx/hooks/codex-native-bridge.py\"",
      "timeout": 10,
